...
class EMRRoles(BaseConstruct):
        ...
        *,
        ...
        if artifacts_bucket:
            artifacts_bucket.grant_read(
                self._instance_role, os.path.join(artifacts_path, "*") if artifacts_path else artifacts_path
            ).assert_success()

        if logs_bucket:
            logs_bucket.grant_read_write(
                self._service_role, os.path.join(logs_path, "*") if logs_path else logs_path
            ).assert_success()
            logs_bucket.grant_read_write(
                self._instance_role, os.path.join(logs_path, "*") if logs_path else logs_path
            ).assert_success()
        ...
        return iam.PolicyDocument(
            statements=[
                iam.PolicyStatement(
                    ...
                    actions=["glue:GetDatabase", "glue:GetDatabases", "glue:GetUserDefinedFunctions"],
                    resources=[
                        stack.format_arn(partition=stack.partition, service="glue", resource="catalog"),
                        stack.format_arn(partition=stack.partition, service="glue", resource="database/default"),
                    ],
                )
            ]
        )
    ...
    def _create_service_role(scope: constructs.Construct, id: str, *, role_name: Optional[str] = None) -> iam.Role:
        role = iam.Role(
            ...
            inline_policies={"emr-artifacts-policy": EMRRoles._emr_artifacts_policy()},
            managed_policies=[
    ...
    def _create_autoscaling_role(scope: constructs.Construct, id: str, *, role_name: Optional[str] = None) -> iam.Role:
        role = iam.Role(
            ...
            assumed_by=iam.ServicePrincipal("elasticmapreduce.amazonaws.com"),
            managed_policies=[
                iam.ManagedPolicy.from_aws_managed_policy_name("service-role/AmazonElasticMapReduceforAutoScalingRole")
            ],
        )
