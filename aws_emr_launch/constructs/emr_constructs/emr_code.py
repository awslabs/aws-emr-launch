    ...
    def resolve(self, scope: constructs.Construct) -> Dict[str, Any]:
        ...
        if self._bucket_deployment is None:
            ...
            self._bucket_deployment = s3_deployment.BucketDeployment(
                scope, f"{self._id}_BucketDeployment" if self._id else "BucketDeployment", **deployment_props
            )
...
class EMRBootstrapAction(Resolvable):
    ...
    def resolve(self, scope: constructs.Construct) -> Dict[str, Any]:
        ...
        return {
            "Name": self._name,
            "ScriptBootstrapAction": {"Path": self._path, "Args": self._args if self._args else []},
        }
