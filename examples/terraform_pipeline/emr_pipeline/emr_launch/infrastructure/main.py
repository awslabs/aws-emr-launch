...
config = {
    'CLUSTER_NAME': app.node.try_get_context('cluster-name')
}
...
if config['CORE_INSTANCE_EBS_SIZE'] > 0:
    assert config['CORE_INSTANCE_EBS_IOPS'] <= config['CORE_INSTANCE_EBS_SIZE'] * 50, \
        f"CORE_INSTANCE_EBS_IOPS must be <=  CORE_INSTANCE_EBS_SIZE (GB) * 50"
...
emr_cluster_stack = EMRClusterDefinition(
    app,
    id=config['CLUSTER_NAME'] + '-stack',
    env=env,
    config=config
)
