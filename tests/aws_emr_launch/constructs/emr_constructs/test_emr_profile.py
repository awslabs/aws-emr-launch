...
def test_profile_authorizations() -> None:
    profile.authorize_input_bucket(input_bucket).authorize_output_bucket(output_bucket).authorize_input_key(input_key)
...
def test_profile_s3_encryption() -> None:
    profile.set_s3_encryption(emr_profile.S3EncryptionMode.SSE_KMS, s3_key)
...
def test_profile_local_disk_encryption() -> None:
    profile.set_local_disk_encryption(local_disk_key, ebs_encryption=True)
...
def test_profile_tls() -> None:
    profile.set_tls_certificate("s3://null_bucket/cert")

    default_profile["TLSCertificateConfiguration"] = {
        "TLSCertificateConfiguration": {"CertificateProviderType": "PEM", "S3Object": "s3://null_bucket/cert"}
    }
...
def test_profile_local_kdc() -> None:
    profile.set_local_kdc(secret)
...
def test_profile_local_kdc_with_cross_realm_trust() -> None:
    profile.set_local_kdc_with_cross_realm_trust(secret, "realm", "domain", "admin_server", "kdc_server")
...
def test_profile_external_kdc() -> None:
    profile.set_external_kdc(secret, "admin_server", "kdc_server")
...
def test_profile_external_kdc_with_cross_realm_trust() -> None:
    profile.set_external_kdc_with_cross_realm_trust(secret, "admin_server", "kdc_server", "ad_realm", "ad_domain")
    ...
    default_profile["KerberosConfiguration"] = {
        ...
        "ExternalKdcConfiguration": {
            ...
            "AdminServer": "admin_server",
            "KdcServer": "kdc_server",
            "AdIntegrationConfiguration": {"AdRealm": "ad_realm", "AdDomain": "ad_domain"},
        },
    }
...
def test_profile_emrfs_prefix_role_mapping() -> None:
    profile.add_emrfs_role_mapping_for_s3_prefixes(emrfs_mappings_role, ["s3://bucket/prefix"])
...
def test_profile_emrfs_user_role_mapping() -> None:
    profile.add_emrfs_role_mapping_for_users(emrfs_mappings_role, ["user"])
...
def test_profile_emrfs_group_role_mapping() -> None:
    profile.add_emrfs_role_mapping_for_groups(emrfs_mappings_role, ["group"])
...
def test_profile_lake_formation_enablement() -> None:
    profile.enable_lake_formation(secret, "s3://bucket/idp.xml", lake_formation_role, services_role)
